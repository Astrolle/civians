...
export const connectMongo = async (): Promise<void> => {
  ...
  await notifications.createIndex({ location: '2dsphere' });
  ...
  await notifications.createIndex({ expires_at: 1 }, { expireAfterSeconds: 0 }); // TTL index
  ...
  await reports.createIndex({ expires_at: 1 }, { expireAfterSeconds: 0 }); // TTL index

  // ── Collection Centers ────────────────────────────────────────────────────
  const centers = mongoDB.collection('collection_centers');
  await centers.createIndex({ location: '2dsphere' });
